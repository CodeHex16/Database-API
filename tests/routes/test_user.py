...
from app.schemas import UserCreate,UserDelete,UserUpdate, UserAuth, UserUpdatePassword,UserForgotPassword
...
async def test__unit_test__update_user_no_edit(fake_user_repo, monkeypatch):
    user_new_data = UserUpdate(_id="notfound@error.com", password="newpassword", is_initialized=True, remember_me=True, scopes=AccessRoles.USER)
    with pytest.raises(HTTPException) as excinfo:
        await update_user(user_new_data, current_user, fake_user_repo)
    assert excinfo.value.status_code == 404
...
@pytest.mark.asyncio
async def test__unit_test__update_password_old_password_error(fake_user_repo, monkeypatch):
    user_data = UserUpdatePassword(password="Abc123!@#@js", current_password="test_password2")
